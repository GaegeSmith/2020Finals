...
class Maths:
    ...
    def basicStats(listOfNums):
        ...
        aveNum = Maths.ave(listOfNums)
        rngNum = Maths.rag(listOfNums)
...
class Terminal:
    ...
    def inputChecker(ui, reqInput):
        ...
        if reqInput == "int":
            ...
            if copyInput.lower() in reqInput:
                match = True
        
        elif type(reqInput) == str:
            if copyInput.lower() == reqInput:
                match = True
...
class Strings:
    ...
    def lstToStr(listy, splitter = "", addNewLine = True):
        ...
        if type(listy) == dict:
            for i in listy:
                string = i
                if i == list(listy)[len(listy) - 1]:
                    result += string
                else:
                    result += string + splitter
        elif type(listy) in (list, tuple, dict):
            for i in range(len(listy)):
                string = listy[i]
                if type(string) in [list, tuple]:
                    string = Strings.lstToStr(string, " ", False)
                else:
                    string = str(string)
                if i == (len(listy) - 1):
                    result += string
